...
<media-tag src="https://cryptpad.private.coffee/blob/89/8927c9def97d83a95fdcc16a394f4a5d185c5e2076f827fb" data-crypto-key="cryptpad:VRTe7mimTnYXiU5saOuUtdvy3QfpD4WJJk9beRu0qCs="></media-tag>


**1. Multi-Model Ensemble Approach**
- Uses three different AI models (ResNet50, EfficientNetV2B3, InceptionV3) that complement each other
- Each model has different strengths in recognizing hand shapes and patterns
- Results are combined using weighted voting for more reliable predictions
- Models were trained on thousands of hand sign images with MediaPipe preprocessing

**2. Hand Detection & Preprocessing**
- Advanced hand detection using MediaPipe technology
- Smart rotation normalization to handle hands in different orientations
- Multiple fallback mechanisms when detection fails
- Image enhancement techniques to improve quality under various lighting conditions

**3. Interactive Visualization**
- Real-time 3D visualizations showing model confidence and decision-making
- Heatmaps highlighting which parts of the hand the AI is focusing on
- Comprehensive metrics dashboards showing performance across all letters
- Interactive charts for exploring model performance
...
6. System explains potentially confusing letters (like M/N/S) with contextual analysis
...
- Specialized ensemble weights based on research
- Confusion pair analysis for commonly mixed-up letters (M/N/S, D/K, U/V/R)
- Grad-CAM visual explanations showing model attention

**Performance Optimizations:**
- Intelligent frame skipping for smooth video processing
- CPU/GPU optimization to work on various devices
- Model-specific preprocessing to match training conditions
- Temporal smoothing for stable predictions in video

## Practical Features

- No installation required - runs directly in web browser
- Works with any webcam or uploaded image
- Multiple operational modes (high accuracy vs. high speed)
- Detailed explanations when confidence is low
- Visual feedback showing exactly what the AI is "seeing"
- Comprehensive performance statistics for educational purposes
...
## 2. ✓ Page Configuration and Constants
```python
# CONFIGURATION
st.set_page_config(
...
## 4. ✓ Additional Lightweight Fallbacks for Hand Detection
```python
...
## 6. ✓ Runtime Toggles and Configuration
```python
...
## 7. ✓ Hand Detection with MediaPipe (CRITICAL FIX)
```python
...
## 13. ✓ Ensemble Prediction with TTA
```python
...
## 14. ✓ Temperature Scaling for Confidence Calibration
```python
...
## 17. ✓ Full Rotated Overlay Function
```python
...
## 18. ✓ Hand Overlay on Rotated Crop
```python
...
## 19. ✓ Synthetic Heatmap from Landmarks
```python
...
def tab_image_upload():
    ...
    if st.button("🖼️ Fetch URL", key='fetch_url'):
        if url:
            try:
